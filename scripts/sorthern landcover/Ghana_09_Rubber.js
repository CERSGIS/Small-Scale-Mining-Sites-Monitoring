...
var palmClass = ee.Image('projects/landcover-mit/assets/Ghana_Palm').select('classification')
...
var classMask = woodyCropsClass.unmask(base)/*.or(wetlandClass.unmask()).not()*/.clip(aoi);
// Map.addLayer(classMask, {palette:['white','blue']}, 'Class Mask', false);

// Mask the composite we are going to use for classification
var compositeMasked = composite.updateMask(classMask)/*.updateMask(palmClass.unmask(base).not().clip(aoi))*/;
...
var bands = ['GV', 'B1', 'NIRv', 'B5', 'EVI', 'B10', 'NDFI', 'SR65', 'TCG']

...
var waterClass = ee.Image('projects/landcover-mit/assets/version_2/Ghana_Mangrove').select('classification');
...
var vegetationMask = composite.select('NDVI').gt(0.75).clip(aoi);
...
var classMask = waterClass.unmask(base).or(mangClass.unmask()).or(miningClass.unmask())
              .or(artSurfClass.unmask()).or(wetlandClass.unmask()).not().clip(aoi);
...
// Map.addLayer(compositeMasked, {bands: ['B4', 'B3', 'B2'], min: 0, max: 0.25, gamma:0.7}, 'True Color Composite', false);
...
// Map.addLayer(compositeMasked, {bands: ['dryB4'], min:-1, max:1, palette: ['red', 'orange','white', 'green', 'blue']}, "Test Index");

// Select the predictors to be used in the Random Forest Classifier
var bands = ['B4','B5','B6','B7','NDVI', 'NDMoI', 'TCG', 'EVI', 'HH', 'RAT1'];

// var bands = ['TCG', 'GCVI', 'B10', 'EVI', 'B3', 'MSAVI', 'HH', 'B7', 'NDFI', 'NDVI'];
...
// Mask Probability to Closed Forest Class
...
Export.image.toAsset({
  image: exportImage,
  description: 'Ghana_ClosedForest',
  scale: 30,
  region: aoi,
  maxPixels:1e13
});
